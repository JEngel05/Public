...
# If the user data is empty, Set StatusCode as BadRequest
...
# If the device is not trusted, Do Nothing.
if (-not($ValidSystem)) {
    Write-Output "Not a valid System"
} else {
    #### Else Start Primary User Evaluation
    ...
    If((-not($IntunePrimaryUser)) -or ($UDAMultiplier -ge 1.5)){
        ...
        $body = "Primary User has been Updated"
            ...
            $OutputMessage = "Validate Azure Managed Identity Permissions and Azure Function Connectivity - $($ResponseInventory), $($ResponseMessage) "
            Write-Warning "$($OutputMessage)"
            #dump error info in stream
            Write-warning "$($error[0].errordetails.message)"
            #Set Response and Automation Fail
            $Body = "$($OutputMessage)"
